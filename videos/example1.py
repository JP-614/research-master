...
def design_envelope_lpf(fs=FS, cutoff=LP_CUTOFF, order=LP_ORDER):
    wn = cutoff / (0.5 * fs)  # 정규화 컷오프
    sos = butter(order, wn, btype='low', output='sos')
    return sos

SOS_LP = design_envelope_lpf()
ZI = [sosfilt_zi(SOS_LP) for _ in range(N_CHANNELS)]  # 채널별 필터 상태
...
def run(port, baud, save_dir, record_seconds):
    ...
    # 실시간 플롯 준비
    fig, axes, imshows = None, None, None

    with serial.Serial(port, baudrate=baud, timeout=1) as ser:
        print(f"[INFO] Serial open: {port} @ {baud}")
        print("[INFO] 인터랙션: 라벨(예: 0,1,2...)을 입력하면 그 라벨로", record_seconds, "초 수집해서 저장합니다.")
        print("[INFO] 'show' 는 계속, 'end' 는 종료.")
        ...
        last_tensor = None  # 최근 텐서(화면 갱신 용)
        recording = False
        rec_label = None
        rec_deadline = 0.0   # 기록 종료 시각(Unix time)
        ...
        while True:
            ...
            if sys.stdin in select_readable():
                ...
                if cmd.lower() == 'end':
                    ...
                    print("[INFO] 실시간 화면만 표시합니다.")
                elif cmd.isdigit():
                    rec_label = int(cmd)
                    rec_deadline = time.time() + float(record_seconds)
                    recording = True
                    print(f"[REC] 라벨={rec_label}, {record_seconds}초 수집 시작!")
                else:
                    print("[INFO] 사용법: 정수라벨 / show / end")
            ...
            try:
                vals = [int(v) for v in parts[1:]]  # 6채널
            except ValueError:
                continue
