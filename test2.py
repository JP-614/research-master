...
if __name__ == '__main__':
    # AIO start
    print("스크립트 실행 시작.") # <-- 추가 1
    print("CONTEC 장치 초기화를 시작합니다...") # <-- 추가 2
    aio = init_contec_and_start(num_of_channels, float(buffer_size))
    
    print("CONTEC 장치 초기화 완료.") # <-- 추가 3
    ...
    try:
        print("이제 입력 대기 루프로 진입합니다.") # <-- 추가 4
        while True:
            # --- 動作番号の入力 (measure スタイル) -----------------------------
            key = input('動作番号を入力:')  # 例: 0,1,2,... / 終了は end
            if key == "end":
                print("学習データ計測終了")
                break
            try:
            ...
            # 3秒採取終了
            aio.stopAi()
            print("Episode captured (3s). 保存を実行...")
            ...
            plt.suptitle(f'Spectrogram for Motion "{prr}"')
            plt.show()
            '''
            #.npzファイルでspectrogram tensor保存
            ts = datetime.datetime.now().strftime("%Y%m%d_%H%M%S_%f")
            np.savez_compressed(
                os.path.join(spec_save_dir, f"spec_ep_{ts}_3s_label{prr}.npz"),
                spec=spec_ep, fs=fs, label=prr,
                learning_time=learning_time,
                STFT_NPERSEG=STFT_NPERSEG, STFT_NOVERLAP=STFT_NOVERLAP
            )
            print(f"保存完了: CSV({semg.shape[0]}行) + NPZ(spec shape={spec_ep.shape}, label={prr})")
    '''
