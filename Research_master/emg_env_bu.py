...
class emg_env(gym.Env):
    def __init__(self, data_dir, train=True, window_points=800, stride_points=100):
        ...
        if not file_list:
            raise ValueError(f"'{data_dir}' 경로에 ep_*.npz로 시작하는 NPZ 파일이 없습니다.")
        # 파일 순서가 OS마다 다를 수 있으므로, 먼저 이름순으로 정렬하여 재현성을 확보합니다.
# 2. 데이터 섞기 로직 (핵심 수정 부분)
        if train:
            print("[INFO] 데이터를 라벨별로 균등하게 섞습니다 (Round-Robin)...")
            files_by_label = defaultdict(list)
            
            # (1) 파일을 라벨별로 분류
            for f_path in file_list:
                fname = os.path.basename(f_path)
                match = re.search(r'label(\d+)', fname) # 파일명에서 'label숫자' 찾기
                label = int(match.group(1)) if match else -1
                files_by_label[label].append(f_path)
            
            # (2) 각 라벨 안에서는 순서 랜덤 섞기
            ...
            # (3) 라벨별로 하나씩 번갈아 가며 뽑기 (0, 1, 2, 0, 1, 2...)
            balanced_list = []
            ...
            print(f"[INFO] 총 {len(self.npz_files)}개 파일 정렬 완료.")
